...
import { getMissionCards, setMissionCards } from "@/lib/db";
...
export default function MissionPage() {
    const [items, setItems] = useState<MissionCard[]>([]);
    const [saved, setSaved] = useState(false);

  useEffect(() => {
        getMissionCards().then(setItems);
  }, []);
  ...
  const handleReset = async () => {
        ...
        setItems(defaultMissionCards);
  };

  return (
        <AdminShell title="المهمة والرؤية والقصة">
              <div className="max-w-2xl">
                      <p className="text-brand-charcoal-light text-sm mb-6">
                                هذه البطاقات تظهر في قسم المهمة والرؤية بالصفحة الرئيسية.
                      </p>
